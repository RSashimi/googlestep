...
class HashTable:
    ...
    def delete(self, key):
        ...
        #------------------------#
# Calculate the hash of the key and take modulo with bucket_size to find which bucket the key belongs to.
        bucket_index = calculate_hash(key) % self.bucket_size
...
# if the current node's key matches the one we want to delete
        while item:
            if item.key == key:
# if there's a previous node, skip the current node by setting prev.next to item.next
                if prev:
                    prev.next = item.next
# if the node to be deleted is the first node in the list, update the bucket’s head to the next node
                else:
                    self.buckets[bucket_index] = item.next
# decrease the total number of stored items by 1
                self.item_count -= 1
# move to the next node, remembering the current one as prev
                return True
            prev = item
            item = item.next        
# If no node with the matching key was found in the list, return False       
        return False
